...
drop function if exists public.hm_login(text);
drop function if exists public.hm_public_state();
drop function if exists public.hm_save_prediction(uuid, text, text, integer, integer);
drop function if exists public.hm_set_result(uuid, text, text, integer, integer);
...
create or replace function public.hm_login(p_invite_code text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if found_player.id is null then
    raise exception 'Boðskóðinn fannst ekki.';
  end if;

  return jsonb_build_object(
    'id', found_player.id,
    'name', found_player.name,
    'isAdmin', found_player.is_admin
  );
end;
$$;

create or replace function public.hm_public_state()
returns jsonb
language sql
security definer
set search_path = public
...
create or replace function public.hm_save_prediction(
  p_player_id uuid,
  p_invite_code text,
  p_match_id text,
...
begin
  ...
  if valid_player.id is null then
    raise exception 'Boðskóðinn passar ekki við leikmann.';
  end if;
  ...
  if selected_match.id is null then
    raise exception 'Leikurinn fannst ekki.';
  end if;

  if selected_match.starts_at <= now() or selected_match.status = 'finished' then
    raise exception 'Þessi leikur er læstur.';
  end if;

  insert into predictions (player_id, match_id, home_goals, away_goals, updated_at)
  values (p_player_id, p_match_id, p_home_goals, p_away_goals, now())
...
create or replace function public.hm_set_result(
...
as $$
declare
  admin_player players%rowtype;
begin
  select *
  into admin_player
  from players
  where id = p_admin_player_id
    and upper(invite_code) = upper(trim(p_invite_code))
    and is_admin = true;

  if admin_player.id is null then
    raise exception 'Aðeins admin getur skráð úrslit.';
  end if;
...
grant execute on function public.hm_login(text) to anon, authenticated;
grant execute on function public.hm_public_state() to anon, authenticated;
grant execute on function public.hm_save_prediction(uuid, text, text, integer, integer) to anon, authenticated;
grant execute on function public.hm_set_result(uuid, text, text, integer, integer) to anon, authenticated;

insert into public.players (name, invite_code, is_admin) values
  ('Hafdís', 'HAFDIS-ADMIN', true),
  ('Leikmaður 1', 'HM-1001', false),
  ('Leikmaður 2', 'HM-1002', false),
  ('Leikmaður 3', 'HM-1003', false);

insert into public.matches (id, starts_at, stage, group_name, home_team, away_team, venue)
select
  fields[1],
  fields[2]::timestamptz,
  fields[3],
  fields[4],
  fields[5],
  fields[6],
